...
in
{
  networking.networkmanager = optionalAttrs isNixOS {
    dns = "systemd-resolved";
    unmanaged = [
      "iphone0"
      "android0"
    ];
    ensureProfiles.profiles = inputs.nixfigs-networks.networks.all // inputs.nixfigs-networks.networks.work // lib.optionalAttrs (config.networking.hostName == "MORPHEUS-LINUX") inputs.nixfigs-networks.wwan;
